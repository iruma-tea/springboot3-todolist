...
public class TodoPdf extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
        ...
        document.add(new Paragraph("ToDo List　", font_g16_bold));
        document.add(new Paragraph("　", font_g16_bold));
        ...
        for (Todo todo : todoList) {
            ...
            document.add(new Paragraph(todo2paragraph(todo), font_g12));
            for (Task task : todo.getTaskList()) {
                ++numOfTask;
                if (task.getDone().equals("Y")) {
                    ++numOfDoneTask;
                }
                document.add(new Paragraph("\t" + task2paragraph(task), font_g10));
            }
        }
        // Todoベースの完了率
        int todoDoneRate;
        ...
        // テーブル定義
        document.add(new Paragraph(" ", font_g12));
        document.add(new Paragraph("　集計表　", font_g12_underline));
